...
import { RuleForgeEngine, RuleExtractor, RuleValidator, YamlFormatter } from './src/index';
...
async function demo() {
  ...
  try {
    ...
    console.log('❌ 规则提取失败:', error instanceof Error ? error.message : error);
  }
  ...
  try {
    ...
    console.log('❌ 完整流程失败:', error instanceof Error ? error.message : error);
  }
