...
import { z } from 'zod';
import { RuleSchema, RuleYAML } from '../types/rule-schema';
...
export class RuleValidator {
  ...
  private validateCompatibility(rule: RuleYAML): string[] {
    ...
    if (rule.compatibility.frameworks && rule.compatibility.frameworks.length > 0) {
      const hasVersionConstraints = rule.compatibility.frameworks.some(fw => 
        rule.compatibility.min_version || rule.compatibility.max_version
      );
    ...
    if (rule.compatibility.tools && rule.compatibility.tools.length > 0) {
      ...
      const unsupportedTools = rule.compatibility.tools.filter(tool => 
        !supportedTools.includes(tool.toLowerCase())
      );
