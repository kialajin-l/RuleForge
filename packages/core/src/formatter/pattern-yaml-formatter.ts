...
export class PatternYamlFormatter {
  private readonly defaultOptions: Required<PatternFormatOptions> = {
    indent: 2,
    lineWidth: 80,
   ...
   * 将模式格式化为 YAML 字符串
   */
  async toYAML(pattern: Pattern, options: PatternFormatOptions = {}): Promise<PatternFormatResult> {
   ...
   * 批量格式化多个模式
   */
  async toYAMLBatch(patterns: Pattern[], options: PatternFormatOptions = {}): Promise<PatternFormatResult[]> {
    const results: PatternFormatResult[] = [];
   ...
   * 预处理模式数据
   */
  private preprocessPattern(pattern: Pattern, options: Required<PatternFormatOptions>, warnings: string[]): Pattern {
   ...
   * 路径脱敏处理
   */
  private sanitizePatternPaths(pattern: Pattern, projectName: string, warnings: string[]): void {
    // 处理触发器中的文件路径
   ...
   * 优化代码示例
   */
  private optimizeCodeExamples(pattern: Pattern, warnings: string[]): void {
    const maxLines = 15;
   ...
   * 截断代码示例
   */
  private truncateCodeExample(lines: string[], maxLines: number): string {
   ...
   * 过滤敏感信息
   */
  private filterSensitiveInfo(pattern: Pattern, warnings: string[]): void {
    // 过滤 API Keys 和密码
    ...
    pattern.examples.forEach(example => {
      ['before', 'after'].forEach(key => {
        const code = example[key as keyof typeof example];
        if (typeof code === 'string') {
          sensitivePatterns.forEach(patternRegex => {
            if (patternRegex.test(code)) {
              example[key as keyof typeof example] = code.replace(patternRegex, '***');
              foundSensitiveInfo = true;
            }
          });
        }
   ...
   * 将模式转换为规则对象
   */
  private patternToRule(pattern: Pattern): any {
   ...
   * 生成包含示例的建议内容
   */
  private generateSuggestionWithExamples(pattern: Pattern): string {
   ...
   * 生成兼容性信息
   */
  private generateCompatibility(pattern: Pattern): any {
   ...
   * 生成 YAML 内容
   */
  private generateYaml(ruleObject: any, options: Required<PatternFormatOptions>): string {
    // 创建深拷贝以避免修改原始对象
    const processedObject = JSON.parse(JSON.stringify(ruleObject));
    
    const yamlString = YAML.stringify(processedObject, {
    ...
    const fixedYaml = yamlString.replace(
      dateRegex,
      '$1$2: "$3"'
    );
   ...
   * 生成文件头注释
   */
  private generateHeaderComment(ruleObject: any): string {
...
`;
   ...
   * 生成文件名
   */
  private generateFileName(pattern: Pattern): string {
...
// 导出便捷函数
export const patternToYAML = async (pattern: Pattern, options?: PatternFormatOptions) => {
  const formatter = new PatternYamlFormatter();
  return formatter.toYAML(pattern, options);
};

export const patternsToYAMLBatch = async (patterns: Pattern[], options?: PatternFormatOptions) => {
  const formatter = new PatternYamlFormatter();
