...
import { SessionLogParser } from './log-parser';
import { PatternClusterer, Pattern } from './pattern-cluster';
import { PatternYamlFormatter } from '../formatter/pattern-yaml-formatter';
...
export class RuleExtractor {
  ...
  private readonly patternClusterer: PatternClusterer;
  private readonly yamlFormatter: PatternYamlFormatter;

  constructor() {
    this.logParser = new SessionLogParser();
    this.patternClusterer = new PatternClusterer();
    this.yamlFormatter = new PatternYamlFormatter();
  }
   ...
   * 解析会话日志
   */
  private async parseSessionLog(logPath: string, options: ExtractOptions) {
   ...
   * 模式聚类分析
   */
  private async clusterPatterns(events: any[], options: ExtractOptions) {
   ...
   * 生成 YAML 规则文件
   */
  private async generateYamlRules(patterns: Pattern[], options: ExtractOptions) {
   ...
   * 将模式转换为 RuleYAML 对象
   */
  private patternsToRuleYAML(patterns: Pattern[], options: ExtractOptions): RuleYAML[] {
   ...
   * 创建空结果
   */
  private createEmptyResult(startTime: number, warnings: string[]) {
    return {
      rules: [],
      yamlFiles: [],
      statistics: {
   ...
   * 批量提取多个会话的规则
   */
  async extractBatch(sessions: Array<{ sessionId: string; logPath: string }>, options: Omit<ExtractOptions, 'sessionId' | 'logPath'> = {}) {
    const results: Array<{ sessionId: string; result: ExtractionResult }> = [];
   ...
   * 生成提取报告
   */
  generateExtractionReport(results: Array<{ sessionId: string; result: ExtractionResult }>): string {
   ...
   * 验证规则质量
   */
  validateRuleQuality(rule: RuleYAML): { valid: boolean; score: number; issues: string[] } {
    const issues: string[] = [];
    let score = 100;
...
// 导出便捷函数
export const extractRules = async (options: ExtractOptions) => {
  const extractor = new RuleExtractor();
  return extractor.extract(options);
};

export const extractRulesBatch = async (sessions: Array<{ sessionId: string; logPath: string }>, options?: Omit<ExtractOptions, 'sessionId' | 'logPath'>) => {
  const extractor = new RuleExtractor();
