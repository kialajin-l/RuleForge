...
export interface ParseSessionLogOptions {
  ...
  strictMode?: boolean;
  onProgress?: (progress: { bytesRead: number; totalBytes: number; eventsParsed: number }) => void;
}
...
export class SessionLogParser {
  ...
  private readonly supportedFormats = ['jsonl', 'vscode', 'trae'] as const;

  /**
   * 解析会话日志文件
   */
  async parseSessionLog(logPath: string, options: ParseSessionLogOptions = {}): Promise<ParseResult> {
   ...
   * 验证文件存在性和大小
   */
  private async validateFile(logPath: string, maxFileSize?: number): Promise<void> {
    try {
   ...
   * 检测日志格式
   */
  private async detectLogFormat(logPath: string): Promise<LogFormatDetector> {
    try {
   ...
   * 检测 JSON Lines 格式
   */
  private isJsonlFormat(content: string): boolean {
   ...
   * 检测 VSCode 格式
   */
  private isVscodeFormat(content: string): boolean {
   ...
   * 检测 Trae 格式
   */
  private isTraeFormat(content: string): boolean {
    // Trae 日志可能包含特定标识
    const traePatterns = [
      /trae/i,
      /TraeAI/,
   ...
   * 解析 JSON Lines 格式
   */
  private async parseJsonlFormat(logPath: string, options: ParseSessionLogOptions): Promise<SessionEvent[]> {
   ...
   * 解析 VSCode 格式（待实现）
   */
  private async parseVscodeFormat(logPath: string, options: ParseSessionLogOptions): Promise<SessionEvent[]> {
   ...
   * 解析 Trae 格式（待实现）
   */
  private async parseTraeFormat(logPath: string, options: ParseSessionLogOptions): Promise<SessionEvent[]> {
    // TODO: 实现 Trae 日志格式解析
   ...
   * 标准化事件数据
   */
  private standardizeEvent(eventData: any, sessionId?: string): SessionEvent | null {
   ...
   * 标准化事件类型
   */
  private standardizeEventType(rawType: string): string {
   ...
   * 提取会话元数据
   */
  private extractSessionMetadata(events: SessionEvent[], logPath: string, sessionId?: string): SessionMetadata {
    if (events.length === 0) {
      return {
    ...
    // 检测 IDE 类型
    let ideType: SessionMetadata['ideType'] = 'unknown';
    const hasVscodeEvents = events.some(event => 
      event.type.includes('vscode') || event.payload?.source === 'vscode'
    );
    const hasTraeEvents = events.some(event => 
      event.type.includes('trae') || event.payload?.source === 'trae'
    );
    
    if (hasVscodeEvents) ideType = 'vscode';
    if (hasTraeEvents) ideType = 'trae';
   ...
   * 提取项目路径
   */
  private extractProjectPath(logPath: string): string | undefined {
    try {
   ...
   * 生成统计信息
   */
  private generateStatistics(events: SessionEvent[], parseTime: number, logPath: string) {
   ...
   * 批量解析多个会话日志
   */
  async parseMultipleSessions(logPaths: string[], options: ParseSessionLogOptions = {}): Promise<ParseResult[]> {
   ...
   * 验证事件数据完整性
   */
  validateEvent(event: SessionEvent): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
...
// 导出便捷函数
export const parseSessionLog = async (logPath: string, options?: ParseSessionLogOptions): Promise<ParseResult> => {
  const parser = new SessionLogParser();
  return parser.parseSessionLog(logPath, options);
};

export const parseMultipleSessions = async (logPaths: string[], options?: ParseSessionLogOptions): Promise<ParseResult[]> => {
  const parser = new SessionLogParser();
