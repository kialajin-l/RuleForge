...
export async function initCommand(options) {
    ...
    try {
        ...
        await fs.writeFile(path.join(rulesDir, 'rules-index.json'), JSON.stringify(indexContent, null, 2));
        steps.complete();
        // 输出成功信息
        logger.newline();
...
# 模板: ${Object.keys(TEMPLATES).find(key => TEMPLATES[key] === config) || 'default'}
 ...
 * 生成 YAML 规则内容
 */
function generateYamlRule(rule) {
