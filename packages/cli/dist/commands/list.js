...
export async function listCommand(options) {
    const { format = 'table', tag = [], framework, language, minConfidence } = options;
    ...
    try {
        ...
        if (filters.framework) {
            filteredRules = filteredRules.filter(rule => rule.compatibility.frameworks?.includes(filters.framework));
        }
        if (filters.language) {
            filteredRules = filteredRules.filter(rule => rule.compatibility.languages.includes(filters.language));
        }
        if (filters.minConfidence) {
            filteredRules = filteredRules.filter(rule => rule.confidence >= filters.minConfidence);
        }
