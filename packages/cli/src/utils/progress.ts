...
export class ProgressBar {
   ...
   * 增加进度
   */
  increment(step: number = 1, currentItem?: string): void {
    if (this.bar) {
...
export class MultiProgressBar {
  ...
  addBar(id: string, title: string, total: number): ProgressBar {
    ...
    // 重写进度条方法以使用多进度条
    progressBar['start'] = () => {};
    progressBar['update'] = (current: number, currentItem?: string) => {
 ...
 * 创建简单的文本进度指示器
 */
export function createTextProgress(total: number, title: string = '处理中'): {
  update: (current: number, currentItem?: string) => void;
  complete: () => void;
} {
  ...
  return {
    update(current: number, currentItem?: string) {
 ...
 * 创建步骤进度指示器
 */
export function createStepProgress(steps: string[]) {
  let currentStep = 0;
